...
function creaJsonMenuIndicadoresT(padre) {
    ...
    $.each(jsonMenu, function (val, item) {
        ...
        menuIndicadoresM += '<li ' + mInd + '><a data-i="' + val + '" data-p="' + item.padre + '" data-a="' + c + '">' + i + '.- ' + item.descripcion + arrow + '</a></li>\n';
        i++;
    })
...
function obtenYmuestraAllDatos() {
    ...
    try {
        ...
        var dataJ = "{fechaF:'" + fechaF + "', idDisJuz:'" + idDisJuz + "', materia:'" + nomJuz + "'}";
...
function obtenYmuestraDatos(jsonActual) {
    ...
    try {
        ...
        var dataJ = "{ind:'" + jsonActual.id + "', fechaF:'" + fechaF + "', idDisJuz:'" + idDisJuz + "', materia:'" + nomJuz + "'}";
        ...
        if (result != null && result.length) {
            ...
            $("#titulo-indicador").text(jsonActual.descripcion + " de juzgados en materia " + $('#Materia option:selected').text());
...
function obtenYmuestraDatosTabla(jsonActual) {
    ...
    try {
        ...
        var dataJ = "{ind:'" + jsonActual.id + "', fecha:'" + fechaF + "', distrito:'" + idDisJuz + "', materia:'" + materia + "'}";
...
function mesAnio(fec) {
    var fech = fec.split("-");
    return Meses[parseInt(fech[1])] + " " + fech[0];
}

function promIndice(json, columna, id) {

    if (json[columna].total != '' && json[columna].total != '0') {
        return ((parseInt(json[columna].valor) * 100) / parseInt(json[columna].total)).toFixed(2);
    } else {
...
function gTabla(jr, id, idx) {
    ...
    var tabla = "<div class=\"container contenedor-tabla-pj\" style=\"width:auto\">"
        ...
        + "<tr>"
        + "<th>Juicio</th> <th>Tiempo en horas</th>"
        + "</tr>"

    $.each(jr[idx].jucios, function (index, item) {
        tabla += "<tr><td>" + item.juicio + "</td><td>" + item.valor +"</td></tr>"
    })
...
function gDona(jr, id, idx) {
    ...
    } else if ((/\b(9|14|15)\b/).test(String(id))) {
...
function eventosClick() {
    $("#Indicadores-materia").on("click", "ul > li", function () {
        if ($(this).children("a").attr("data-a") == "y") {
            pestActual = "indi";
            if ((/id(15)/).test($(this).children("a").attr("data-i"))) {
                obtenYmuestraDatosTabla(obtenJson(this));
            } else {
                obtenYmuestraDatos(obtenJson(this));
            }
    ...
    $("#logo-pj").click(function () {
        window.open("http://web2.pjedomex.gob.mx/");
    })

    /*$("body").on("click", ".sub-m-ind > li", function () {
        if ($(this).children("a").attr("data-a") == "y") {
            pestActual = "indi";
            obtenYmuestraDatos(obtenJson(this));
        } else {
...
function muestraSubMenu(ele) {
    ...
    $.each(jsonActual.hijos, function (val, item) {
        ...
        subMenuActivo += '<li><a data-i="' + val + '" data-p="' + item.padre + '" data-a="' + c + '">' + i + '.- ' + item.descripcion + '</a></li>\n';
        i++;
    })
...
function eventosMouseover() {
   /* 
    $("#Indicadores-materia").on("mouseover", "ul > li", function () {

        $(".sub-m-ind").remove();
        jsonActual = obtenJson(this);

        if (jsonActual.submenu == 'y') {
            ...
            var ul = document.createElement("ul");
            ul.innerHTML = subMenuActivo;
            ul.style.backgroundColor = "#9d2424";
